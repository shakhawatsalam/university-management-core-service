...
const insertIntoDB = async (
  ...
  const isCourseOngoingComplete = await prisma.studentEnrolledCourse.findFirst({
    where: {
      OR: [
  ...
  if (isCourseOngoingComplete) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      `There is already an ${isCourseOngoingComplete.status?.toLocaleLowerCase()} registration`
    );
  }
